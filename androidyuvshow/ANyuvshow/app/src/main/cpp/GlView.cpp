...
H264YUV_Frame* frame = 0;
void initGL(){
    //glClearColor(0.1f, 0.4f, 0.6f, 1.0f);
    if(frame == 0){
        frame = (H264YUV_Frame*)malloc(sizeof(H264YUV_Frame));
        frame->luma.dataBuffer = (unsigned char*)malloc(2048*2048);
        frame->chromaR.dataBuffer = (unsigned char*)malloc(2048*2048);
        frame->chromaB.dataBuffer = (unsigned char*)malloc(2048*2048);
    }
    LOGE("=====LINENUM = %d======funname = %s frame = %p\n",__LINE__,__FUNCTION__,frame);
...
int setYUV420N21(int width,int heigh, char* buf,int size){
    pthread_mutex_lock(&mutex_lock);
    int datasize_y = width*heigh;
    frame->height = heigh;
    frame->width = width;
    memcpy(frame->luma.dataBuffer,buf,datasize_y);
    frame->luma.length = datasize_y;
    //提取vu分量
    for (int i = 0,k = 0; i < datasize_y/4; i++,k += 2) {
        frame->chromaR.dataBuffer[i] = buf[datasize_y + k];
        frame->chromaB.dataBuffer[i] = buf[datasize_y + k +1];
        frame->chromaB.length = datasize_y/4;
        frame->chromaR.length = datasize_y/4;
    }
    pthread_mutex_unlock(&mutex_lock);
    return 0;
}